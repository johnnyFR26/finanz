...
import { AddCategoriesModalComponent } from '../add-categories/add-categories-modal.component';
...
export class EditTransactionModalComponent{
    readonly dialogRef = inject(MatDialogRef<EditTransactionModalComponent>);
    ...
    protected value = signal<number>(this.data.transaction.value);
    protected description = signal(this.data.transaction.description);
    protected type = signal(this.data.type);
    protected categoryId = signal<string | null>(this.data.transaction.category.id)
    ...
    onSubmit(): any {
      console.table(this.formValue())
      this.transactionService.editTransaction(this.formValue())
      this.dialogRef.close()
    }
